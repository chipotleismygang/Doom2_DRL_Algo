...
import time  # For tracking elapsed time in episodes
...
class DoomTASEnv(Env):
    ...
    def __init__(self, config: Dict[str, Any], render: bool = False):
        ...
        # episode_start_time: timestamp when episode started (for 1-hour limit)
        self.episode_start_time = time.time()
        
        # max_episode_seconds: maximum seconds an episode can run (1 hour = 3600 seconds)
        self.max_episode_seconds = 3600
    ...
    def _action_to_buttons(self, action: np.ndarray) -> list:
        ...
        # All buttons use the same threshold of 0.5 (must be > 50% to activate)
        # This is consistent with the keyboard override in TeacherForcingWrapper
    ...
    def reset(self, seed=None, options=None) -> Tuple[Dict, dict]:
        ...
        # Record episode start time for 1-hour limit
        self.episode_start_time = time.time()
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict, float, bool, bool, dict]:
        ...
        # Episode ends if: map is finished (reached exit) OR player died OR 1-hour time limit exceeded
        elapsed_seconds = time.time() - self.episode_start_time
        done = (self.game.is_episode_finished() or 
                self.game.is_player_dead() or 
                elapsed_seconds > self.max_episode_seconds)
        ...
        info = {
            ...
            "elapsed_seconds": elapsed_seconds  # Time elapsed in episode
        }
...
class TASSBEvaluationCallback(BaseCallback):
    ...
    4. Advances to next level when agent completes current level
    ...
    def _get_current_map(self) -> str:
        """
        Get the map string for the current level.
        Doom 2 maps are named MAP01, MAP02, etc.
        """
        return f"MAP{self.current_level:02d}"
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps % 10000 == 0 and self.num_timesteps > 0:
            ...
            if success_count > 0:
                ...
                print(f"   Level: {self._get_current_map()}")
                ...
                # If success rate is very high, advance to next level
                if success_rate >= 0.66:  # 2 out of 3 episodes successful
                    self.current_level += 1
                    self.best_time = float('inf')  # Reset best time for new level
                    
                    # Update the training and eval environments to new level
                    new_map = self._get_current_map()
                    print(f"\n🎉 ADVANCING TO {new_map}!")
                    
                    # Reload environments with new map
                    self.model.env.close()
                    self.eval_env.close()
                    
                    # Recreate with new level (would need config update in practice)
                    # For now, just reset the level counter or handle appropriately
...
def train_tas_agent():
    ...
    5. Automatically progresses through levels
    """
